    ...
    return (
        <>
            <div className="flex flex-col items-center bg-purple-100 w-96 p-10 mx-auto mt-16 gap-4 drop-shadow-lg">
                ...
                {topics.map((topic) => {
                    return <Link className="text-2xl underline underline-offset-8 font-bold text-orange-500" href={`/topics/${topic.topicId}`} key={topic.topicId}>{topic.topicName}</Link>
                })}
                <br />
